...
class ModeloFormularios
{
	...
	static public function mdlRegistroProveedor($files)
	{
		$stmt = Conexion::conectar()->prepare("INSERT INTO proveedor (archivos) VALUES (?)");
		$stmt->execute(array($files));

		$stmt = Conexion::conectar()->prepare("SELECT MAX(idSupplier) AS id FROM proveedor WHERE nombreComercial IS NULL");
		$stmt->execute();
		$id = $stmt->fetch();
		return $id["id"];
		$stmt = null;
	...
	static public function mdlUpdateProveedor($data, $id)
	{
		$stmt = Conexion::conectar()->prepare("UPDATE proveedor SET nombreFiscal = ?, nombreComercial = ?, negocio = ?, numeroCliente =?,
												rfc = ?, correo = ?, direccion = ?, ciudad = ?, estado = ?, codigoPostal = ?, numeroTel = ?,
												tipoCompañia = ?, terminoPago = ?, tipotrans = ?, provfijo = ? WHERE idSupplier = ?");
		$res = $stmt->execute(array(
			$data["nombreFiscal"], $data["nombreComercial"], $data["negocio"], $data["numeroCliente"], $data["rfc"],
			$data["correo"], $data["direccion"], $data["ciudad"], $data["estado"], $data["codigoPostal"], $data["numeroTel"],
			$data["tipoCompania"], $data["terminoPago"], $data["tipoTransferencia"], $data["provFijo"], $id
		));
		return $res;
		$stmt = null;
	}
	...
	Registrar nueva PO
	=============================================*/
	static public function mdlRegistrarPO($data, $idUsuario, $provfijo)
	{
		//Se obtiene la longitud del array para iterar la query
		$arr_length = count($data["itemNumber"]);

		//Se inserta la nueva PO evaluando si es un proveedor fijo. Si lo es, se aprueba la compra inmediatamente
		if ($provfijo == "SI")
			$stmt = Conexion::conectar()->prepare("INSERT INTO compras (idUsuario, idSupplier, fechaPo, divisa, IVA, estatus) VALUES (?, ?, CURDATE(), ?, ?, 'APROBADO')");
		else
	...
	static public function mdlActualizarFechaPago($data)
	{
		...
		$stmt->execute(array($data["fechaPago"], $data["idPO"]));
		return $stmt->fetch();
		$stmt = null;
	}
}
